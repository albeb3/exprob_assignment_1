...
class ArucoNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.image_pub = self.create_publisher(Image, 'image_with_circle', 10)
        self.pos_marker_x_on_camera_pub = self.create_publisher(Float32, 'pos_marker_x_on_camera', 10)
        ...
        # Set up variables for handling the markers
        self.marker_list=[]
        self.trigger_show= None
        # Set up variable mission
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            for i, marker_id in enumerate(marker_ids):
                ...
                center = np.mean(corners[i], axis=1).astype(int)  # Calculate center of marker corners
                center = tuple(center.flatten())  # Flatten and convert to a tuple
                
                pose = Pose()
                pose.position.x = tvecs[i][0][0]
                pose.position.y = tvecs[i][0][1]
                pose.position.z = tvecs[i][0][2]
                #self.get_logger().info(f"posizione x del token : {pose.position.x}")
                #self.get_logger().info(f"posizione y del token : {pose.position.y}")
                #self.get_logger().info(f"posizione z del token : {pose.position.z}")
                rot_matrix = np.eye(4)
                ...
                #cv2.waitKey(4)  # This will keep updating the window
                self.image_pub.publish(self.bridge.cv2_to_imgmsg(cv_image, encoding="rgb8"))
            
            vel_msg = Float32()
            vel_msg.data = pose.position.x  # Imposta il valore della posizione
            self.pos_marker_x_on_camera_pub.publish(vel_msg)   
